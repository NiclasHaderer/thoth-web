...
const wrapFetch = <K extends ("author" | "series" | "book")>(
    ...
    key: K,
    fetchFunction: (offset: number, limit: number) => Promise<PaginatedResponse<INITIAL_STATE[`${K}Map`][string]> | undefined>
) => {
...
const wrapUpdate = <K extends ("author" | "series" | "book"), UPDATE_TYPE extends { id: string }>(
    mutate: Mutate<StoreApi<INITIAL_STATE>, [StoreMutatorIdentifier, unknown][]>,
    key: K,
    updateFunction: (data: UPDATE_TYPE) => Promise<INITIAL_STATE[`${K}Map`][string] | undefined>
) => {
...
const wrapSorting = <K extends ("author" | "series" | "book")>(
    mutate: Mutate<StoreApi<INITIAL_STATE>, [StoreMutatorIdentifier, unknown][]>,
    key: K,
    sortingFunction: (offset: number, limit: number) => Promise<string[] | undefined>
) => {
    return async (offset: number) => {
        const limit = 30
        const offsetCount = offset * limit
        ...
        mutate.setState(state => ({
            ...state,
            [`${key}Sorting`]: replaceRangeInList(state[`${key}Sorting`], offsetCount, response)
        }))
    }
}
...
const wrapDetails = <K extends ("author" | "series" | "book")>(
    mutate: Mutate<StoreApi<INITIAL_STATE>, [StoreMutatorIdentifier, unknown][]>,
    key: K,
    detailsFunction: (id: string) => Promise<INITIAL_STATE[`${K}Map`][string] | undefined>
) => {
...
const wrapSortingOf = <K extends ("author" | "series" | "book")>(
    mutate: Mutate<StoreApi<INITIAL_STATE>, [StoreMutatorIdentifier, unknown][]>,
    key: K,
    sortingFunction: (id: string) => Promise<Position | undefined>,
) => {
    return async (id: string) => {
        ...
        mutate.setState(state => ({
            ...state,
            [`${key}Sorting`]: state[`${key}Sorting`].filter(i => i !== id).splice(response.sortIndex, 0, id)
        }))
    }
